...
export default function NewPost() {
    ...
    const [files, setFiles] = useState([]);       // ⬅️ new
    ...
    const submit = async (e) => {
        ...
        setError("");
        try {
            const fd = new FormData();                 // ⬅️ use FormData
            fd.append("title", title);
            fd.append("body", body);
            files.forEach((f) => fd.append("files", f)); // ⬅️ field name must be "files"
                ...
                <div>
                    ...
                    {files.length > 0 && (
                        <ul className="mt-2 text-sm opacity-80 list-disc pl-5">
                            {files.map((f) => (
                                <li key={f.name}>{f.name} <span className="opacity-60">({Math.ceil(f.size / 1024)} KB)</span></li>
                            ))}
                        </ul>
                    )}
                </div>
